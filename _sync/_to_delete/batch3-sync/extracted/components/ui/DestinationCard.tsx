...
import { getTripCountForDestination } from "@/lib/destinations-catalog";
...
export function DestinationCard({
  ...
  const count = getTripCountForDestination(destination.name);
  const label = count === 0 ? "No trips yet" : `${count} trip${count === 1 ? "" : "s"} available`;
    ...
    <Link href={`/destinations/${destination.slug}`} className="group block">
      <div className={`relative w-full ${height} overflow-hidden rounded-[14px] bg-surface-hover transition-shadow group-hover:shadow-[0_8px_20px_-8px_oklch(20%_0.02_255/0.18)]`}>
        <Image
          ...
          alt={`${destination.name}, ${destination.category.replace("-", " ")} destination`}
          fill
