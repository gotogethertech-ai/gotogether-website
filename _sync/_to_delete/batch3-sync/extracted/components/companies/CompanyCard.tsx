...
import { formatRatingWithBasis, type CompanyData } from "@/lib/companies-data";

/**
 * Company Card, per the blueprint's spec: logo → name+badge → one-line
 * description → trips-run + rating-with-basis row. Whole card links to
 * the profile. No promotional variation between cards (Concept C).
 */
export function CompanyCard({ company }: { company: CompanyData }) {
  return (
    ...
    >
      ...
      <div className="mb-2.5 truncate text-[11px] text-text-tertiary">{company.description}</div>
