...
export type Review = {
  id: string;
  /** Real reviewer's user id, when known — used to link to their real
   * /profile/{id} page instead of the legacy name-encoded mock link. */
  reviewerId?: string;
  reviewerName: string;
...
export type ProfileData = {
  ...
  name: string;
  initials: string;
  /** Public Supabase Storage URL for the user's uploaded profile photo.
   * Optional/undefined on the legacy mock records below (they predate
   * real photo upload) — every consumer falls back to the initials
   * circle when this is unset, same as SessionUser/AuthUser's avatarUrl. */
