    ...
    trip.status === "draft"
      ...
      : trip.status === "completed"
        ...
        : trip.status === "cancelled"
          ? "View Trip"
          : "Manage Trip";
  // Draft/live/in-progress route into Host Trip Management; a Cancelled
  // trip's "View Trip" goes to the read-only public page; a Completed
  // trip goes straight to reviewing co-travellers (the organizer is
  // equally eligible to review/be reviewed — submit_trip_review makes no
  // distinction between organizer and regular accepted member).
  const primaryHref =
    trip.status === "completed"
      ? `/trips/${trip.tripId}/review`
      : trip.status === "cancelled"
        ? `/trips/${trip.tripId}`
