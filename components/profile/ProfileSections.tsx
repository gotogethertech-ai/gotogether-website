  ...
  return (
    <div className="flex flex-col gap-4 min-[600px]:flex-row min-[600px]:items-start">
      <Avatar
        ...
        size={96}
        alt={hasPhoto ? `${profile.name}'s photo` : ""}
        className="text-xl"
      ...
      {photoOpen && hasPhoto && (
        ...
        >
          ...
          <button
            aria-label="Close"
            onClick={() => setPhotoOpen(false)}
            className="absolute right-5 top-5 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-lg font-bold text-text-primary hover:bg-white"
          >
            ×
          </button>
        </div>
